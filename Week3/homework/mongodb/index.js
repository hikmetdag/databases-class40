...
async function findEpisodesExercises(client) {
  ...
  const { title } = await client.db("databaseWeek3").collection("bob_ross_episodes").findOne({ episode: "S02E02" });
  ...
  const { episode } = await client.db("databaseWeek3").collection("bob_ross_episodes").findOne({ title: "BLACK RIVER" });
  console.log(
    `The season and episode number of the "BLACK RIVER" episode is ${episode}`
  );

  // Find all of the episode titles where Bob Ross painted a CLIFF [Should be: NIGHT LIGHT, EVENING SEASCAPE, SURF'S UP, CLIFFSIDE, BY THE SEA, DEEP WILDERNESS HOME, CRIMSON TIDE, GRACEFUL WATERFALL]
  const result1 = await client.db("databaseWeek3").collection("bob_ross_episodes").find({ elements: "CLIFF" });
  const result1a = await result1.toArray();
  const result1b = result1a.map(result => result.title);
  console.log(
    `The episodes that Bob Ross painted a CLIFF are ${result1b}`
  );

  // Find all of the episode titles where Bob Ross painted a CLIFF and a LIGHTHOUSE [Should be: NIGHT LIGHT]

  const result2 = await client.db("databaseWeek3").collection("bob_ross_episodes").find({ elements: { $all: ["CLIFF", "LIGHTHOUSE"] } });
  const result2a = await result2.toArray();
  const result2b = result2a.map(result => result.title);
  console.log(
    `The episodes that Bob Ross painted a CLIFF and a LIGHTHOUSE are ${result2b}`
  );
}
...
async function updateEpisodeExercises(client) {
  ...
  const result1 = await client.db("databaseWeek3").collection("bob_ross_episodes").updateMany({ elements: "BUSHES" }, { $set: { "elements.$[element]": "BUSH" } }, { arrayFilters: [{ element: "BUSHES" }] });
  console.log(
